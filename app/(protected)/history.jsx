  ...
  const callFastlaneHistoryApi = async (serviceID) => {
    try {
      ...
      const prepared = data.map((item) => ({
        ...item,
        _exit: item.exit_date ? Date.parse(item.exit_date) : 0,
      }));

      // Sort by numeric timestamp (MUCH faster)
      const sortedData = prepared.sort((a, b) => b._exit - a._exit);
      ...
      showToast({
        ...
        message:
          error?.error ||
          error?.message ||
          "Failed to get service history data",
        sessionExired: error?.error == "Another session is active.",
        logout: logout,
      });
    } finally {
