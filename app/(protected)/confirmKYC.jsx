...
export default function VerifySuccess() {
    ...
    const renderIcon = () => {
        if (result?.data?.transaction_status?.toUpperCase() == "SUCCESS") {
            return (<CheckCircle style={{ marginBottom: 10 }} color={COLORS.profitColor} size={80} />)
        }
        return (
            <Image
    ...
    const renderButton = () => {
        if(result?.data?.transaction_status?.toUpperCase() !== "SUCCESS"){
             return (
            <Button buttonStye={{ marginHorizontal: 20 }} onClick={() => { router.push("forms/kyc") }} label={"Try Again"} gradientColor={['#D36C32', '#F68F00']} />
        )
        }
        return (
            <Button buttonStye={{ marginHorizontal: 20 }} onClick={() => { router.push("home")}} label={"Done"} gradientColor={['#D36C32', '#F68F00']} />
        )
    }
    ...
    return (
        <ScrollView contentContainerStyle={styles.container}>
            <Header />
            {loading && <ActivityIndicator style={{marginTop: 80, marginBottom: 20}} size="large" color="#007bff" />}
            ...
            {result && (
                <View style={styles.card}>
                    ...
                    <Text style={[styles.label]}>Transaction Status: <Text style={{ color: getStatusColor(result.data?.transaction_status) }}>{result.data?.transaction_status}</Text></Text>
